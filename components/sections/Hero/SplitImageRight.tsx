...
/** Split hero — text on the left, image on the right. Falls back to the
 *  shared `<GradientPlaceholder/>` when no image is provided so unresolved
 *  cues still render a designed surface rather than a blank box. */
export function HeroSplitImageRight({ title, subtitle, eyebrow, ctaText, ctaHref, image }: Props) {
  return (
...
function HeroImage({ image }: { image?: HeroSpec['image'] }) {
  if (image?.src) {
    return (
      // eslint-disable-next-line @next/next/no-img-element -- Next/Image needs domain config; raw <img> keeps this server-pure for now.
      <img
        src={image.src}
        alt={image.alt ?? ''}
